...
# Raíz del módulo IA. Se mantiene aquí (no en _ai.py) porque los consumidores
# y tests la leen/parchean vía cloud.services.AI_BASE_ROOT; _ai.py la resuelve
# en tiempo de llamada desde este módulo.
...
# Frontera Cloud→Core (fase 6M.1): la resolución de rutas del restore vive en
# core.cloud_paths (lógica pura). Cloud inyecta su BASE_CLOUD_ROOT y re-exporta
# safe_join; resolve_restore_destination / user_root_for_uid son wrappers finos.
...
def user_root_for_uid(uid):
    ...
    return _core_user_root_for_uid(uid, BASE_CLOUD_ROOT)
...
def resolve_restore_destination(uid, target_rel_path):
    """Compat: delega en core.cloud_paths inyectando el BASE_CLOUD_ROOT de Cloud."""
    return _core_resolve_restore_destination(uid, target_rel_path, BASE_CLOUD_ROOT)
